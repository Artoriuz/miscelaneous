...
os.mkdir("low_res")
os.system(f"magick mogrify -colorspace rgb -filter box -resize {int(10000 / scale)}% -colorspace srgb -path low_res {args.path}*.png")
...
for resampling_filter in tqdm(resampling_filters):
    ...
    if EWA:
        os.system(f"magick mogrify -colorspace RGB +sigmoidal-contrast 7.5 -filter {resampling_filter} -distort Resize {scale}% -sigmoidal-contrast 7.5 -colorspace sRGB -path high_res ./low_res/*.png")
    else:
        os.system(f"magick mogrify -colorspace RGB +sigmoidal-contrast 7.5 -filter {resampling_filter} -resize {scale}% -sigmoidal-contrast 7.5 -colorspace sRGB -path high_res ./low_res/*.png")
    
    ref_filelist = sorted(glob.glob(args.path + "*.png"))
    test_filelist = sorted(glob.glob("high_res/*.png"))
